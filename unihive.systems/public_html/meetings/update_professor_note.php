...
// Debug incoming data
error_log("Update note data: " . print_r($data, true));

if (!isset($data['noteId']) || !isset($data['noteContent']) || !isset($data['timestamp'])) {
    header('Content-Type: application/json');
...
$stmt = $conn->prepare("
    ...
    SET note_content = ?, timestamp = ?, is_public = ?, note_category = ?, note_color = ?
    WHERE id = ? AND professor_id = ?
");

$stmt->bind_param("sdissii", $noteContent, $timestamp, $isPublic, $noteCategory, $noteColor, $noteId, $professorId);
